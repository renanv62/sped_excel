...
def ECD_ICMS(ecd_icms_path, dado, batch_size=20):
    data = ''
    ...
    with tqdm(total=len(ecd_icms_list), desc="Processando arquivos", bar_format="{desc}") as pbar:
        for i in range(0, len(ecd_icms_list), batch_size):
            batch_paths = ecd_icms_list[i:i + batch_size]

            for path in batch_paths:
                try:
                    replace_ecd_icms_str = path.replace("/", "\\")

                    with open(replace_ecd_icms_str, 'r', encoding="latin-1") as file:
                        # Bloco 0
                        cont_0000 = {}
                        cont_0001 = {}
    ...
    if dado == '1':
        ...
        wb.save("D:\\Documents\\app\\test_ecd_ICMS.xlsx")
